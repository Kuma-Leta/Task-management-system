...
const DARK_BG = "#1F2937"; // Darker, smoother background
...
const Sidebar = ({ userRole }) => {
  ...
  const menuItems = [
    { path: "/todos", label: "To-Dos", icon: <FaTasks /> },
    { path: "/completed", label: "Completed", icon: <FaCheckCircle /> },
    { path: "/tasks", label: "Tasks", icon: <FaTasks /> },
    { path: "/in-progress", label: "In Progress", icon: <FaSpinner /> },
  ];
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-3 z-50 p-2 rounded-lg md:hidden shadow-lg"
        style={{ backgroundColor: PRIMARY_COLOR, color: LIGHT_TEXT }}
        ...
        <nav className="flex flex-col p-4 gap-2 flex-grow">
          {menuItems.map((item, index) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `block px-4 py-3 rounded-lg transition-all duration-200 font-medium flex items-center gap-3
            ...
            >
              ...
              {isCollapsed && <span className="ml-1">{item.label[0]}</span>}
            </NavLink>
          ))}
        </nav>
